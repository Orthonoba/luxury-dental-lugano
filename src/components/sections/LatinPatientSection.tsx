...
export default function LatinPatientSection() {
  const t = useTranslations('latin')

  const features = [
    { icon: '🇨🇭', text: t('features.certified') },
    { icon: '💬', text: t('features.language') },
    { icon: '✈️', text: t('features.international') },
    { icon: '⭐', text: t('features.stars') },
  ]

  const whatsappUrl = `${SITE_CONFIG.contact.whatsapp}?text=${encodeURIComponent('Buongiorno, vorrei ricevere informazioni sui trattamenti dentali presso Luxury Dental Paradiso, Lugano.')}`

  return (
    <section className="py-32 bg-beige overflow-hidden relative" aria-label={t('eyebrow')}>
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        <div className="grid lg:grid-cols-5 gap-16 lg:gap-20 items-center">
          ...
          >
            ...
            >
              {t('headline')}
              <span className="block text-olive">{t('headlineGold')}</span>
            </motion.h2>
          ...
          <div className="lg:col-span-2 relative hidden lg:flex items-center justify-center min-h-[480px]">
            ...
            >
              ...
              <p className="text-luxury-black text-sm font-semibold">{t('idiomaValue')}</p>
            </motion.div>
            ...
            >
              ...
              <p className="text-white text-sm font-semibold">📍 {t('locationValue')}</p>
            </motion.div>

            {/* Floating card — attention */}
            <motion.div
              variants={fadeUp}
              initial="hidden"
              whileInView="visible"
