...
import { dentalBookingSchema, type DentalBookingInput, DENTAL_SPECIALTIES } from '@/lib/validations/booking'
...
export default function DentalBookingForm() {
  ...
  } = useForm<DentalBookingInput>({
    resolver: zodResolver(dentalBookingSchema),
    defaultValues: {
      firstName: '',
      lastName: '',
      email: '',
  ...
  const onSubmit = async (data: DentalBookingInput) => {
    setServerError(null)
    try {
      const res = await fetch('/api/appointments/dental', {
        method: 'POST',
    ...
    return (
      ...
      >
        <motion.div
          ...
          className="w-20 h-20 rounded-full bg-olive/10 flex items-center justify-center mb-6"
        >
          <svg viewBox="0 0 24 24" fill="none" stroke="#607C3A" strokeWidth={1.5} className="w-8 h-8">
        ...
        <p className="text-luxury-black/50 max-w-sm text-sm leading-relaxed">
          Hemos recibido su solicitud de cita en Odontología Avanzada. Le confirmaremos la disponibilidad en las
  ...
  return (
    ...
    >
      ...
      <motion.div variants={fadeUp}>
        ...
        <Input label="Fecha de nacimiento (opcional)" id="dateOfBirth" type="date"
          error={errors.dateOfBirth?.message} {...register('dateOfBirth')} />
      </motion.div>

      {/* 02 — Specialty */}
      <motion.div variants={fadeUp}>
        <span className={sectionLabel}>02 — Especialidad</span>
        <div className="mb-5">
          <label htmlFor="service" className={labelClass}>Especialidad dental</label>
          <select
            id="service"
        ...
        <Textarea
          ...
          id="notes"
          rows={4}
          placeholder="Cuéntanos sobre tu caso o cualquier información relevante…"
